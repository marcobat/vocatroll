...
if (is_array($records)) {
  ...
  foreach ($records as $field) {
    if ($element_set_name != $field['element_set_name']) {
      if ($element_set_name != '') {
        echo('</div>'); // loses <div class="element_set_area"> if a previous one was opened (skips the first)
      }  
      $element_set_name = $field['element_set_name'];
      echo('<div class="element_set_area"><h3>'.$field['element_set_name'].'</h3>'); 
    }
    //print_r($field);
  ...
  echo('<p>'.__('No Custom Element Set Found').'</p>');
